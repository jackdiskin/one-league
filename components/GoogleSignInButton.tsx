...
export function GoogleSignInButton({ callbackURL = "/dashboard" }: { callbackURL?: string }) {
  const onClick = async () => {
    // BetterAuth typically supports either `redirectTo` or `callbackURL`.
    // Use the one your version expects.
    await authClient.signIn.social({
      provider: "google",
      callbackURL, // if your API expects `callbackURL`, swap the key
    });
  };
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="mt-4 cursor-pointer inline-flex items-center justify-center gap-2 rounded-md border px-8 py-2 text-sm font-medium hover:bg-gray-50"
    >
